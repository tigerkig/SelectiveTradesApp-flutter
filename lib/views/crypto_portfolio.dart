...
class CryptoPortfolioScreen extends StatefulWidget {
  ...
  @override
  _CryptoPortfolioScreenState createState() => _CryptoPortfolioScreenState();
}
...
class _CryptoPortfolioScreenState extends State<CryptoPortfolioScreen> {
  ...
  Widget title_text = const Text("Crypto portfolio");
  ...
  Widget build(BuildContext context) {
    if(is_searching){
      ...
      title_text =  Text("Crypto portfolio");
    }
    return Scaffold(
      backgroundColor: Colors.white,
      floatingActionButton: FloatingActionButton(
        onPressed: (){
          Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) => AddPortfolioScreen(from: Constants.crypto_portfolio, callback: callback,)));
        },
  ...
  Future<void> getPortfolios() async {
    ...
    if(is_connected){
      ...
      portfolios.clear();
      await db_helper.clearPortfolioTable(Constants.crypto_portfolio);
      final params = {
        "user": user.email,
        "table": Constants.crypto_portfolio
      };
      try{
  ...
  Future<void> getPortfolioSQLite() async {
    ...
    portfolios = await db_helper.getCryptoPortfolioSQLite();
    portfolios.sort((a, b){
  ...
  Future<void> getQuote() async{
    ...
    for(var i = 0; i<portfolios.length; i++){
      try{
        ...
        List<String> s = portfolios[i].symbol.split("/");
        ...
        if(response.statusCode == 200){
          if(response.body.isNotEmpty){
            ...
            try{
              ...
              print("crypto_portfolio.getQuote error getting request status: ${e.toString()}");
            }
            if(quote == null){
              ...
              var pct_net_change = (net_change / p.buy_price) * 100;
            ...
            await db_helper.saveCryptoPortfolioSQLite(p);
          ...
          Container(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index){
                ...
                return PortfolioAdapter(portfolio: p, from: Constants.crypto_portfolio, callback: callback,);
              },
            ),
          ),
          portfolios.isEmpty ? noItem("Press '+' to add a crypto pair\n and swipe down to refresh", context) : Container()
        ],
      ),
    );
  }
